...
<div class="min-h-screen">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition 
                        ...
                        <div class="bg-gradient-to-r from-red-500 to-orange-500 px-4 py-3">
                            <h3 class="text-white font-semibold">⚠️ Emergency Requests ({{ $unreadEmergencyRequestsCount }})</h3>
                        </div>
                        <div class="max-h-48 overflow-y-auto">
                            @foreach($unreadEmergencyRequests as $emergency)
                            <a href="{{ route('admin.emergency') }}" class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-100 transition-colors">
                                <div class="flex items-start gap-3">
                                    <div class="w-10 h-10 bg-gradient-to-br from-red-400 to-orange-400 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                        ⚠️
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="font-semibold text-gray-800 truncate">
                                            {{ $emergency->teacher->first_name ?? 'Unknown' }} {{ $emergency->teacher->last_name ?? '' }}
                                        </p>
                                        <p class="text-sm text-gray-500 truncate">{{ Str::limit($emergency->reason, 40) }}</p>
                                        <p class="text-xs text-gray-400 mt-1">{{ $emergency->start_date }} - {{ $emergency->end_date }}</p>
                        ...
                        <div class="flex border-t border-gray-200">
                            @if($unreadEmergencyRequestsCount > 0)
                            <a href="{{ route('admin.emergency') }}" class="flex-1 px-4 py-3 text-center text-red-600 hover:bg-red-50 font-medium transition-colors border-r border-gray-200">
                                View Emergencies →
                            </a>
                            @endif
                            @if($unreadRequestsCount > 0)
                            <a href="{{ route('admin.requests') }}" class="flex-1 px-4 py-3 text-center text-pink-600 hover:bg-pink-50 font-medium transition-colors">
                                View Requests →
                            </a>
                            @endif
                        </div>
    ...
    <!-- Emergency Request Alert Banner -->
    @if($unreadEmergencyRequestsCount > 0)
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div class="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-xl p-4 flex items-center gap-4 shadow-sm">
            ...
            <div class="flex-1">
                <h4 class="font-semibold text-red-800">You have {{ $unreadEmergencyRequestsCount }} new emergency request{{ $unreadEmergencyRequestsCount > 1 ? 's' : '' }}!</h4>
                <p class="text-sm text-red-700">Review and handle pending emergency absence requests.</p>
            </div>
            <a href="{{ route('admin.emergency') }}" class="bg-red-500 hover:bg-red-600 text-white font-semibold px-4 py-2 rounded-lg transition-all">
                Review Now
            </a>
        </div>
    </div>
