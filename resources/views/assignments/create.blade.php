    ...
    <div class="relative z-10 w-full max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-gradient-to-br from-pink-50/90 via-white/90 to-pink-100/90 backdrop-blur-sm shadow-2xl rounded-3xl p-8 lg:p-10 border-2 border-pink-200/50">
            ...
            <h2 class="text-3xl font-black text-gray-800 mb-8 flex items-center gap-3">
                ...
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-pink-600 to-rose-600">Create New Assignment</span>
            </h2>
            ...
            <form method="POST" action="{{ route('assignments.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="border-2 border-pink-200 rounded-xl px-4 py-3 w-full focus:ring-2 focus:ring-pink-400 focus:border-pink-400 bg-white transition-all" placeholder="Enter assignment title" required>
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="due_date" value="{{ old('due_date') }}" class="border-2 border-pink-200 rounded-xl px-4 py-3 w-full focus:ring-2 focus:ring-pink-400 focus:border-pink-400 bg-white transition-all" required>
                        @error('due_date')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="border-2 border-pink-200 rounded-xl px-4 py-3 w-full focus:ring-2 focus:ring-pink-400 focus:border-pink-400 bg-white transition-all" placeholder="Enter assignment description">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-4 pt-4 border-t-2 border-pink-200">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 bg-gradient-to-r from-pink-400 to-rose-400 hover:from-pink-500 hover:to-rose-500 text-white px-10 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border-2 border-pink-300/50">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                        </svg>
                        Upload Assignment
